...
def hemisphere_image(browser):
    ...
    try:
        ...
        num_pics = len(main_page.select('div.item'))
        # Loop over the link of each image
        for i in range(num_pics):
            #Create an empty dict to hold the search results
            results = {}
            # Find link to picture and open it
            link_image = main_page.select("div.description a")[i].get('href')
            ...
            url_image = image_page.select_one("div.downloads ul li a").get('href')
            #Get the full image title
            url_image = f'https://marshemispheres.com/{url_image}'
            title = image_page.select_one("h2.title").get_text()
            #Add to the results dict
            results = {"url_image": url_image, "title": title}
